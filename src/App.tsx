...
import {
  ArrowRight, Check, CheckCircle2, ChevronDown, Gift, Instagram, Menu, Phone,
  Send, Sparkles, X, XCircle,
} from 'lucide-react'
import { advantages, audience, childBenefits, faqs, formats, parentBenefits, solution, steps } from './data'
import { realPhotos } from './photos'

const scrollToForm = () => document.querySelector('#request')?.scrollIntoView({ behavior: 'smooth' })
...
function Header() {
  ...
  const links = [['О нас', 'about'], ['Форматы', 'formats'], ['Как это работает', 'steps'], ['Галерея', 'gallery'], ['FAQ', 'faq']]
  return <header className="header">
    <div className="container header-inner">
      <Logo />
      <nav className="desktop-nav">{links.map(([label, id]) => <a key={id} href={`#${id}`}>{label}</a>)}</nav>
      <div className="header-actions"><a className="phone" href="tel:+73831234567"><Phone size={17} /> +7 (383) 123-45-67</a><button className="btn small" onClick={scrollToForm}>Рассчитать праздник</button></div>
      <button className="menu-btn" aria-label={open ? 'Закрыть меню' : 'Открыть меню'} onClick={() => setOpen(!open)}>{open ? <X /> : <Menu />}</button>
    </div>
    {open && <nav className="mobile-nav" aria-label="Мобильная навигация">{links.map(([label, id]) => <a key={id} href={`#${id}`} onClick={() => setOpen(false)}>{label}</a>)}<a href="tel:+73831234567">+7 (383) 123-45-67</a></nav>}
  </header>
}
...
function Faq() {
  ...
  return <section id="faq" className="section"><div className="container card faq"><Title>Ответы на частые вопросы</Title>
    {faqs.map(([q, a], i) => <div className="faq-item" key={q}><button aria-expanded={open === i} onClick={() => setOpen(open === i ? null : i)}>{q}<span>{open === i ? '−' : '+'}</span></button>{open === i && <p>{a}</p>}</div>)}
  </div></section>
}
...
function LeadForm() {
  ...
  const submit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!data.get('name') || !data.get('phone') || !data.get('consent')) {
      setError('Укажите имя и телефон, затем подтвердите согласие на обработку данных.')
      return
    }
    setError(''); setSent(true)
  }
  return <section id="request" className="section"><div className="container lead-card">
    <div className="lead-copy"><Title center={false}>Хотите устроить ребёнку праздник, который он будет <span>вспоминать?</span></Title><p>Оставьте заявку — мы свяжемся с вами и подберём лучший вариант праздника!</p><Photo index={5} /></div>
    {sent ? <div className="success"><CheckCircle2 /><h3>Заявка принята!</h3><p>Это демонстрационная форма. Менеджер Mister Teen уже мысленно подбирает праздник.</p></div> :
    <form onSubmit={submit} noValidate>
      <input name="name" placeholder="Ваше имя" />
      <input name="phone" placeholder="Телефон или Telegram" />
      <select name="format" defaultValue=""><option value="" disabled>Какой праздник планируете?</option>{formats.map(([name]) => <option key={name}>{name}</option>)}</select>
      <div className="form-row"><select name="age" defaultValue=""><option value="" disabled>Возраст ребёнка</option><option>5–8 лет</option><option>9–12 лет</option><option>13–15 лет</option></select><input name="guests" type="number" placeholder="Кол-во гостей" /></div>
      <input name="date" type="date" />
      <label className="consent"><input name="consent" type="checkbox" /> Согласен на обработку персональных данных</label>
      {error && <p className="form-error">{error}</p>}
      <button className="btn wide" type="submit">Получить расчёт праздника <ArrowRight /></button>
    </form>}
  </div></section>
}
...
export default function App() {
  return <><Header /><main id="top">
    <section className="hero"><div className="container hero-grid"><div className="hero-photo"><img src={realPhotos[4]} alt="Настоящий праздник Mister Teen" /></div><div className="hero-copy"><p className="eyebrow">Детские праздники под ключ</p><h1><span>MISTER TEEN</span><br />возвращается</h1><p className="hero-sub">И снова создаёт детские праздники под ключ <b>в Новосибирске</b> — ярко, весело и без хлопот.</p><div className="hero-buttons"><button className="btn" onClick={scrollToForm}>Рассчитать праздник <ArrowRight /></button><a className="btn outline" href="#certificate"><Gift /> Получить 10 000 бонусных рублей</a></div></div></div></section>
    ...
    <Faq /><LeadForm />
  </main><footer><div className="container footer"><Logo /><div className="socials"><a href="#top" aria-label="Telegram"><Send /></a><a href="#top" aria-label="Instagram"><Instagram /></a><a href="#top" aria-label="Праздники"><Sparkles /></a></div><div><a href="tel:+73831234567"><b>+7 (383) 123-45-67</b></a><p>Новосибирск</p></div></div><div className="container legal"><span>© Mister Teen, 2011–2026</span><a href="#top">Политика конфиденциальности</a><a href="#top">Публичная оферта</a></div></footer></>
}
